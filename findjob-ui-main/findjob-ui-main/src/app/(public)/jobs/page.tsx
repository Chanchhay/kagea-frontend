      ...
      <main className="min-h-screen bg-white text-slate-950 dark:bg-[#181B1C] dark:text-[#F5F5F5]">
        <div className="mx-auto max-w-[120rem] px-5 py-8 sm:px-8 lg:px-12 lg:py-12 xl:px-16 2xl:px-24">
          <section ref={searchPanelRef} aria-label={t("findJobsPage.searchAria")} className="relative grid gap-2 overflow-hidden rounded-[22px] border border-slate-200 bg-white p-2 md:grid-cols-[1.2fr_1.15fr_1fr_1.25fr_auto] md:items-stretch dark:border-[#3E444B] dark:bg-[#22262C]">
            ...
            <label className="group flex min-h-15 items-center gap-3 rounded-2xl px-3.5 transition-colors hover:bg-slate-50 focus-within:bg-slate-50 dark:hover:bg-[#2B3036] dark:focus-within:bg-[#2B3036]">
              <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition-colors group-focus-within:bg-[#E9F7EB] group-focus-within:text-[#1FA628] dark:bg-[#2B3036] dark:text-[#929AA3] dark:group-focus-within:bg-[#1FA628]/10 dark:group-focus-within:text-[#75D47C]"><BriefcaseBusiness className="size-4" /></span>
              <span className="min-w-0 flex-1"><span className="block text-xs font-bold uppercase tracking-[.08em] text-slate-400 dark:text-[#7F8995]">{t("jobs.experience")}</span>
              <Select value={experience || null} onValueChange={(value) => changeFilter(() => setExperience(value ?? ""))}>
                <SelectTrigger className="-ml-3 h-8 w-full border-none px-3 font-medium hover:border-none focus-visible:ring-0">
                  <SelectValue placeholder={t("findJobsPage.anyExperience")} />
                </SelectTrigger>
            ...
            <label className="flex min-h-15 flex-col justify-center rounded-2xl px-4 transition-colors hover:bg-slate-50 focus-within:bg-slate-50 dark:hover:bg-[#2B3036] dark:focus-within:bg-[#2B3036]">
              <span className="flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-[.08em] text-slate-400 dark:text-[#7F8995]"><span>{t("jobs.minimumSalary")}</span><span className="rounded-full bg-[#FFF5CE] px-2 py-0.5 text-xs text-[#9A7400] dark:bg-[#F3BE00]/12 dark:text-[#F3BE00]">${minimumSalary.toLocaleString()}{minimumSalary === 0 ? "+" : ""}</span></span>
              <input aria-label={t("findJobsPage.minimumSalaryAria")} type="range" min="0" max={salaryCeiling} step="250" value={minimumSalary} onChange={(event) => changeFilter(() => setMinimumSalary(Number(event.target.value)))} className="mt-2 h-1.5 cursor-pointer accent-[#F3BE00]" />
            </label>
            <button type="button" className="inline-flex h-11 items-center justify-center gap-2 self-center rounded-xl bg-[#008A1E] px-6 text-sm font-medium text-white transition-colors hover:bg-[#007018] md:mx-1"><Search className="size-4" />{t("findJobsPage.search")}</button>
          </section>
          ...
          <div className="mt-8 grid items-start gap-8 lg:mt-12 lg:grid-cols-[240px_minmax(0,1fr)]">
            ...
            <section>
              ...
              {jobsQuery.isLoading || facetsQuery.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-56 animate-pulse rounded-2xl bg-slate-100 dark:bg-[#22262C]" />)}</div> :
                jobsQuery.isError || facetsQuery.isError ? <ErrorState message={t("findJobsPage.unableToLoad")} /> : jobs.length ?
                <>
                  <div className={`grid gap-4 transition-opacity sm:grid-cols-2 xl:grid-cols-3 ${jobsQuery.isFetching ? "opacity-60" : ""}`} aria-busy={jobsQuery.isFetching}>{jobs.map((job) => <JobCard key={job.id} job={job} saved={savedJobs.has(job.id)} onSave={() => setSavedJobs(toggleSet(savedJobs, job.id))} />)}</div>
                  <Pagination page={page} totalPages={totalPages} totalElements={totalElements} pageSize={PAGE_SIZE} onChange={setPage} />
                </> :
                <div className="rounded-2xl border border-dashed border-slate-300 px-6 py-20 text-center dark:border-[#3E444B]"><h2 className="text-lg font-semibold">{t("findJobsPage.noMatchingJobs")}</h2><p className="mt-2 text-sm text-slate-500">{t("findJobsPage.tryChangingFilters")}</p><button type="button" onClick={clearFilters} className="mt-5 rounded-xl bg-[#008A1E] px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#007018]">{t("findJobsPage.clearFilters")}</button></div>}
            </section>
...
function JobCard({ job, saved, onSave }: { job: PublicJobResponse; saved: boolean; onSave: () => void }) {
  ...
  return (
    <article className="group flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-4 transition-colors duration-200 hover:border-[#008A1E] dark:border-[#3E444B] dark:bg-[#22262C] dark:hover:border-emerald-400">
      <div>
        {/* Employer */}
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2.5">
            <span className="flex size-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-[#EEF8F0] text-xs font-semibold text-[#008A1E] dark:border-[#3E444B] dark:bg-[#2B3036] dark:text-[#F3BE00]">
              {initials(job.companyName)}
            </span>

            <div className="min-w-0">
              <p className="truncate text-xs font-medium text-slate-800 dark:text-[#F5F5F5]">{job.companyName}</p>
              <p className="truncate text-[10px] text-slate-400 dark:text-slate-500">{timeAgo(job.publishedAt, t)}</p>
            </div>
          </div>
        ...
        {/* Role */}
        <h2 className="mt-2.5 line-clamp-2 text-sm font-semibold leading-snug tracking-tight text-slate-950 transition-colors group-hover:text-[#008A1E] dark:text-white dark:group-hover:text-[#F3BE00]">
          {job.title}
        </h2>
      ...
      {/* Footer */}
      <div className="mt-3.5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3 dark:border-[#3E444B]/60">
        <p className="min-w-0 truncate text-xs font-semibold text-slate-950 dark:text-[#F5F5F5]">
          {salary(job.salaryMin, job.salaryMax, t)}
        </p>
        <Link
          href={`/jobs/${job.id}`}
          aria-label={`${t("findJobsPage.applyForAria")} ${job.title}`}
          className="inline-flex h-7 shrink-0 items-center justify-center gap-1 rounded-lg bg-[#008A1E] px-3 text-xs font-medium text-white transition-colors hover:bg-[#007018]"
        >
          {t("jobs.apply")}
...
function SearchField({ icon: Icon, topLabel, placeholder, value, onChange }: { icon: typeof Search; topLabel: string; placeholder: string; value: string; onChange: (value: string) => void }) { 
  return (
    <label className="group flex min-h-15 items-center gap-3 rounded-2xl px-3.5 transition-colors hover:bg-slate-50 focus-within:bg-slate-50 dark:hover:bg-[#2B3036] dark:focus-within:bg-[#2B3036]">
      <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition-colors group-focus-within:bg-[#E9F7EB] group-focus-within:text-[#1FA628] dark:bg-[#2B3036] dark:text-[#929AA3] dark:group-focus-within:bg-[#1FA628]/10 dark:group-focus-within:text-[#75D47C]">
        <Icon className="size-4" />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-xs font-bold uppercase tracking-[.08em] text-slate-400 dark:text-[#7F8995]">{topLabel}</span>
        <input type="search" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="w-full bg-transparent text-sm font-medium text-slate-800 outline-none placeholder:text-slate-400 dark:text-[#F5F5F5] dark:placeholder:text-[#7F8995]" />
      </span>
    </label>
  ); 
}

function CompactSearchField({ icon: Icon, label, value, onChange }: { icon: typeof Search; label: string; value: string; onChange: (value: string) => void }) { 
  return (
    <label className="group flex min-w-0 flex-1 items-center gap-3 rounded-xl px-3 py-2 transition-colors hover:bg-slate-50 focus-within:bg-slate-50 dark:hover:bg-[#2B3036] dark:focus-within:bg-[#2B3036]">
      <Icon className="size-4 shrink-0 text-slate-400 transition-colors group-focus-within:text-[#1FA628]" />
      <span className="min-w-0 flex-1">
        <span className="sr-only">{label}</span>
        <input type="search" value={value} onChange={(event) => onChange(event.target.value)} placeholder={label} className="w-full bg-transparent text-sm font-medium text-slate-800 outline-none placeholder:text-slate-400 dark:text-[#F5F5F5] dark:placeholder:text-[#7F8995]" />
      </span>
    </label>
  ); 
}
...
function Tag({ children }: { children: React.ReactNode }) { return <span className="inline-flex min-h-5 items-center rounded-md bg-slate-50 px-2 text-[10px] font-medium text-slate-600 dark:bg-[#2B3036] dark:text-[#CBD0D5]">{children}</span>; }
function formatLabel(value: string) { return value.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase()); }
function salary(min: number | undefined, max: number | undefined, t: (key: string) => string) { if (!min && !max) return t("landing.newestJobs.salaryNegotiable"); const money = (value: number) => `$${new Intl.NumberFormat().format(value)}`; return min && max ? `${money(min)} – ${money(max)}` : min ? `${t("landing.newestJobs.salaryFrom")} ${money(min)}` : `${t("landing.newestJobs.salaryUpTo")} ${money(max!)}`; }
function timeAgo(value: string, t: (key: string) => string) { const time = Date.parse(value); if (Number.isNaN(time)) return t("landing.newestJobs.time.recently"); const days = Math.max(0, Math.floor((Date.now() - time) / 86_400_000)); if (days === 0) return t("landing.newestJobs.time.today"); if (days === 1) return t("landing.newestJobs.time.oneDayAgo"); if (days < 30) return `${days} ${t("landing.newestJobs.time.daysAgo")}`; const months = Math.floor(days / 30); return months === 1 ? t("landing.newestJobs.time.oneMonthAgo") : `${months} ${t("landing.newestJobs.time.monthsAgo")}`; }
function initials(name: string) { return name.split(/\s+/).filter(Boolean).slice(0, 2).map((word) => word[0]).join("").toUpperCase() || "CO"; }
