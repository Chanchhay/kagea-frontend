...
export default function RecruiterJobDetailPage() {
  ...
  return (
    <>
      <PageIntro
        title={job.title}
        description={tx([job.companyName, job.location].filter(Boolean).join(" · "))}
        action={
          <Link
      ...
      <div className="grid gap-6">
        <PlainCard>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <StatusPill>{tx(job.status)}</StatusPill>
        ...
        <PlainCard>
          <dl className="grid gap-5 text-sm sm:grid-cols-2 lg:grid-cols-3">
            <Detail label={tx("Category")} value={job.categoryName} />
            <Detail label={tx("Job type")} value={job.jobType} />
            <Detail label={tx("Work mode")} value={job.workMode} />
            <Detail label={tx("Experience level")} value={job.experienceLevel} />
            <Detail
              label={tx("Salary range")}
              value={
            ...
            />
            <Detail label={tx("Expires on")} value={job.expiredAt?.slice(0, 10)} />
          </dl>
        </PlainCard>
      </div>
    </>
