...
export default function StrokeText({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, (typeof fontSize === 'number' ? fontSize : 128) * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  const viewBox = box
    ...
    : `0 ${-(typeof fontSize === 'number' ? fontSize : 128)} 600 ${(typeof fontSize === 'number' ? fontSize : 128) * 1.3}`;
  ...
  return (
    <span
      ...
      className={[
        ...
        .filter(Boolean)
        .join(' ')}
      style={{ ...style, ['--stroke-text-height' as string]: `${Math.round((typeof fontSize === 'number' ? fontSize : 128) * 1.3)}px` }}
      role="img"
