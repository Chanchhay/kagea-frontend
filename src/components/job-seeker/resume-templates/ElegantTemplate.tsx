...
export function ElegantTemplate({ data, fallbackName }: ResumeTemplateProps) {
  ...
  return (
    <Sheet className="px-16 py-14 font-serif text-lg">
      <header className="flex flex-col items-center border-b pb-7 text-center" style={{ borderColor: `${accent}66` }}>
        ...
        <h1 className="text-4xl font-normal tracking-[0.06em] text-slate-950">{name}</h1>
        {data.professionalTitle ? (
          <p className="mt-2 text-lg uppercase tracking-[0.28em]" style={{ color: accent }}>
            {data.professionalTitle}
          </p>
        ) : null}
        {contacts.length ? (
          <p className="mt-4 max-w-[520px] text-lg leading-5 text-slate-500">{contacts.join("  ·  ")}</p>
        ) : null}
      </header>
            ...
            {data.experience.map((entry) => (
              <div key={entry.id}>
                <div className="flex items-baseline justify-between gap-4">
                  <p className="text-lg text-slate-900">{entry.role}</p>
                  <p className="shrink-0 text-lg italic text-slate-500">{formatDateRange(entry.start, entry.end, entry.current)}</p>
                </div>
                {entry.company || entry.location ? (
                  <p className="text-lg uppercase tracking-[0.14em] text-slate-500">
          ...
          <div className="space-y-4">
            {data.education.map((entry) => (
              <div key={entry.id} className="flex items-baseline justify-between gap-4">
                <div>
                  <p className="text-lg text-slate-900">{entry.degree}</p>
                  {entry.school ? <p className="text-lg uppercase tracking-[0.14em] text-slate-500">{entry.school}</p> : null}
                  <Description text={entry.description} className="mt-1 leading-6 text-slate-600" />
                </div>
                {entry.year ? <p className="shrink-0 text-lg italic text-slate-500">{entry.year}</p> : null}
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title={tx("Projects")} accent={accent}>
          <div className="space-y-4">
            {data.projects.map((project) => (
              <div key={project.id}>
                <p className="text-lg text-slate-900">{project.name}</p>
                {project.url ? <p className="break-all text-lg italic text-slate-500">{project.url}</p> : null}
...
function Section({ title, accent, children }: { title: string; accent: string; children: ReactNode }) {
  return (
    <section className="py-6">
      <h2 className="mb-4 text-center text-lg uppercase tracking-[0.32em] text-slate-800">
        {title}
