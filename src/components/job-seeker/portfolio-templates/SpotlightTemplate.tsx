      ...
      <header className="relative overflow-hidden px-16 pb-14 pt-16">
        ...
        <div className="relative flex items-start gap-8">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-[44px] font-semibold leading-tight tracking-tight text-white">{title}</h1>
            {theme.tagline ? <p className="mt-2 text-[18px] font-medium" style={{ color: accent }}>{theme.tagline}</p> : null}
            {summary ? <p className="mt-5 max-w-[640px] whitespace-pre-line text-[18px] leading-7 text-slate-400">{summary}</p> : null}
            {publicUrl ? (
              <p className="mt-6 inline-flex items-center gap-2 rounded-full px-4 py-2 text-[18px] font-semibold text-white" style={{ background: accent }}>
