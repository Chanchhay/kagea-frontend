      ...
      <header className="px-20 pb-10 pt-20">
        <div className="flex items-center gap-6">
          ...
          <div className="min-w-0">
            ...
            {theme.tagline ? <p className="mt-1.5 text-[18px] text-slate-500">{theme.tagline}</p> : null}
          </div>
        </div>
        {summary ? <p className="mt-8 max-w-[620px] whitespace-pre-line text-[18px] leading-7 text-slate-600">{summary}</p> : null}
        {publicUrl ? (
          <p className="mt-5 text-[18px] font-medium" style={{ color: accent }}>{displayUrl(publicUrl)}</p>
        ) : null}
      </header>
