      ...
      <header className="px-20 pb-10 pt-20">
        <div className="flex items-center gap-6">
          ...
          <div className="min-w-0">
            ...
            {theme.tagline ? <p className="mt-1.5 text-lg text-slate-500">{theme.tagline}</p> : null}
          </div>
        </div>
        {summary ? <p className="mt-8 max-w-[620px] whitespace-pre-line text-lg leading-7 text-slate-600">{summary}</p> : null}
        {publicUrl ? (
          <p className="mt-5 text-lg font-medium" style={{ color: accent }}>{displayUrl(publicUrl)}</p>
        ) : null}
      </header>

      <main className="px-20 pb-20">
        <h2 className="border-t border-slate-200 pt-8 text-lg font-semibold uppercase tracking-[0.22em] text-slate-400">{tx("Projects")}</h2>
        {projects.length ? (
              ...
              <article key={project.id} className="flex gap-7 py-7">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-4">
                    ...
                    <div className="flex shrink-0 gap-4 text-lg font-semibold">
                      {project.projectUrl ? (
                  ...
                  {project.description ? (
                    <p className="mt-2 line-clamp-3 whitespace-pre-line text-lg leading-6 text-slate-600">{tx(project.description)}</p>
                  ) : null}
